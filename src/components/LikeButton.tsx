...
const LikeButton = ({ postId, initialLikes, onLikeChange }: LikeButtonProps) => {
  ...
  const checkIfLiked = async () => {
    ...
    try {
      const { data, error } = await supabase.rpc('check_user_liked_post' as any, {
        p_post_id: postId,
        p_user_id: user.id
      });
  ...
  const handleLikeToggle = async () => {
    ...
    try {
      if (isLiked) {
        ...
        const { error } = await supabase.rpc('unlike_post' as any, {
          p_post_id: postId,
          p_user_id: user.id
        });
        ...
        const { error } = await supabase.rpc('like_post' as any, {
          p_post_id: postId,
          p_user_id: user.id
        });
